...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if mask is not None:
            ...
            attn = attn.masked_fill(~mask.to(x.device), float('-inf'))
...
class TransformerTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([Block(dim=dim, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias) for i in range(depth)]) # TODO: Create a list of transformer blocks and wrap inside nn.ModuleList
...
class TransformerDecoderTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([DecoderBlock(dim=dim, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias) for i in range(depth)]) # TODO: Create a list of transformer decoder blocks and wrap inside nn.ModuleList
